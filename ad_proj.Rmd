...
En tercer lloc, es farà un **Anàlisi Discriminant (LDA/QDA)**. Els objectius principals d'aquest estudi són  L'anàlisi de discriminant lineal (LDA en anglès) consisteix en trobar una funció discriminant tal que decideixi a quina classe $C_k$ pertany una observació $x$.
...
```{r}
hist(Xd[,5], main = "Histograma de residual.sugar")
hist(log(Xd[,5]), main = "Histograma de log(residual.sugar)")
```
